...
export function AmenitiesCatalogTab({ categories, setCategories, onSave, saveStatus }) {
    const updateCategory = (catIndex, patch) => {
        setCategories((prev) =>
            prev.map((c, i) => (i === catIndex ? { ...c, ...patch } : c))
        );
    };
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Kategorie a produkty — tabulky{' '}
                    <code className="text-xs">hotel_supplies_categories</code>,{' '}
                    <code className="text-xs">hotel_supplies_items</code>
                </p>
                        ...
                        {category.items.map((item, itemIndex) => (
                            ...
                            >
                                ...
                                <label className="md:col-span-2 block text-sm">
                                    <span className="text-gray-500 text-xs">Pořadí</span>
                                    <input
                                        type="number"
                                        className={inputClass}
